# tierlane

日本語 | [English](README.md)

[![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/license-MIT-green.svg)](LICENSE)
[![Tests](https://github.com/MRL-mana/tierlane/actions/workflows/test.yml/badge.svg)](https://github.com/MRL-mana/tierlane/actions/workflows/test.yml)

AIの仕事を、内容と費用に合わせて最適なCLIへ振り分けます。ソースファイルをクラウドへ送るときは、明示的な許可が必要です。

依存パッケージなし。設定ファイル1つ。Python 3.11以上。

初期設定を生成した後、AIを呼び出さずに振り分けを確認できます。

```bash
tierlane "translate this changelog to Japanese" --dry-run
tierlane "review the error handling in this module" --dry-run
```

未編集の初期設定では、前者は `selected_tier: 0`（ollama）、後者は
`selected_tier: 2`（claude-haiku）になります。これは振り分け結果であり、AIの実行結果や速度測定ではありません。

**初期設定の判定語は英語です。** 日本語の「レビュー」「要約」などを自動翻訳して判定する機能はありません。
日本語で振り分けたい場合は、後述の「日本語の設定例」を使って判定語を設定してください。

## 解決すること

複数のAI CLIを使っていると、簡単な整形まで高価なモデルへ送りがちです。さらに、ファイルを添付した瞬間に、確認していないコードや認証情報まで外部へ送る危険があります。

tierlaneは、その2つを実行前に判断します。

- 軽い仕事は安価なローカルAIへ
- レビューや設計は必要な上位モデルへ
- ファイル全体を秘密情報パターンで検査
- クラウドへファイルを送る場合は `--allow-cloud` が必須
- 許可したフォルダ外の読み取りを拒否
- 設定した `hold` の判定語に一致する重要作業はAIへ委譲せず保留

## インストール

```bash
pip install https://github.com/MRL-mana/tierlane/releases/download/v0.3.0/tierlane-0.3.0-py3-none-any.whl
```

この方法はGit本体を必要としません。タグ付きソースから導入する場合:

```bash
pip install "tierlane @ git+https://github.com/MRL-mana/tierlane.git@v0.3.0"
```

現在PyPIには未公開です。どちらもGitHubの公開版v0.3.0を導入します。

開発する場合:

```bash
git clone https://github.com/MRL-mana/tierlane
cd tierlane
pip install -e ".[dev]"
pytest
```

## 60秒で試す

設定ファイルを生成します。

```bash
tierlane init
```

既存の `tierlane.toml` は上書きしません。使用していないAIの設定を削除し、まず実行せずに振り分け結果だけ確認します。

```bash
tierlane doctor
```

`doctor` はAIを呼び出さず、設定とCLIコマンドの有無を確認します。HTTP接続先は設定済みとして表示しますが、通信は行いません。その後、振り分け結果を確認します。

```bash
tierlane "このファイルの変更点を要約して" --dry-run
```

`--dry-run` はAIを呼び出しません。ルール調整中も費用や外部送信は発生しません。

上の日本語の「要約して」は、未編集の初期設定では判定語に一致せず、最も安いtier 0になります。
`doctor` の確認や `--dry-run` の成功は、AI本体の接続成功を意味しません。

## ファイル送信の安全確認

```console
$ tierlane "説明して" --files db.py --allow-cloud
[tierlane] blocked: OpenAI key found in db.py — refusing to send it.
```

検査は切り詰める前のファイル全体に対して実行します。GitHub、Slack、OpenAI、AWS、Google、秘密鍵、JWT、`api_key = "..."` 形式などを検出します。

クラウドのAIへファイルを渡す場合:

```bash
tierlane "このコードをレビューして" --files app.py --allow-cloud
```

`--allow-cloud` がない場合は送信せず停止します。依頼文だけでファイルを付けない場合は、この確認の対象外です。

## 日本語の設定例

以下はローカルのollamaとクラウドのclaude-haikuを使う場合の例です。
日本語の判定語を使いたい場合に、自分の `tierlane.toml` へ反映してください。
既存設定へ追記する際は同じlevelのtierを重複定義せず、既存の読み取り範囲や `hold` の条件を保持してください。

**クラウドのtierへ振り分けると、添付なしの依頼文は `--allow-cloud` なしでも外部送信されます。**
まず `--dry-run` で確認し、完全にローカルだけで使いたい場合はクラウドのtierとそのルールを設定から除いてください。

```toml
[[tier]]
level = 0
name = "ollama"
kind = "http"
endpoint = "http://127.0.0.1:11434/v1/chat/completions"
model = "qwen2.5-coder:7b"
cloud = false

[[tier]]
level = 2
name = "claude-haiku"
kind = "cli"
command = ["claude", "-p", "{prompt}", "--model", "haiku", "--output-format", "text"]
cloud = true

[[rule]]
tier = 0
keywords = ["下書き", "翻訳", "整形"]

[[rule]]
tier = 2
keywords = ["レビュー", "設計", "リファクタ"]

[hold]
keywords = [
  "deploy", "delete", "drop table", "force push", "rotate key", "credential", "production",
  "公開", "本番", "削除", "認証情報", "鍵のローテーション",
]
```

複数ルールに一致した場合は、最も高いtierを選びます。一致しない仕事は、設定済みの最も安いtierへ送ります。

`hold` も文字列の部分一致であり、危険な意味を自動理解する機能ではありません。
未編集の初期設定では日本語の「本番へ公開して」は保留されません。上の例では「本番」「公開」を明示的に追加しています。

## 終了コード

| コード | 意味 |
| ---: | --- |
| `0` | 成功 |
| `1` | AIの実行が失敗 |
| `2` | 重要作業のため委譲せず保留 |
| `3` | 設定エラー |
| `4` | 秘密情報・パス・クラウド送信の安全確認で停止 |
| `130` | 中断 |

## ライセンス

[MIT License](LICENSE)
